...
data = []
with open("C:\\Users\\ravip\\Desktop\\CI Project\\PCA_ANN\\input\\test_inp.csv", 'r') as csvfile:
...
eig_vec = []
with open("C:\\Users\\ravip\\Desktop\\CI Project\\PCA_ANN\\output\\eign_vec_ec.csv", 'r') as csvfile2:
...
for i in range(r1):
	for k in range(c2):
		new[i][k] = np.dot(list(map(lambda x: float(x),data[i])), list(map(lambda x: float(x),eig_vec[:,k])))
...
np.savetxt("C:\\Users\\ravip\\pca_inp_test_NO2_ec.csv", new1, delimiter=",")
np.savetxt("C:\\Users\\ravip\\pca_inp_test_O3_ec.csv", new2, delimiter=",")
